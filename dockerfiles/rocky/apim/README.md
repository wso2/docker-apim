...
This section defines the step-by-step instructions to build an [Rocky Linux](https://hub.docker.com/_/rockylinux) based Docker image for WSO2 API Manager 4.0.0.
...
- Download wso2am-4.0.0.zip from [here](https://wso2.com/api-management/install/)
- Host the product pack using a webserver.
- Navigate to `<AM_DOCKERFILE_HOME>` directory. <br>
- Execute `docker build` command as shown below.

```
docker build -t wso2am:4.0.0-rocky .
```
...
```
docker run -it -p 9443:9443 -p 8243:8243 wso2am:4.0.0-rocky
```
...
```
docker run -it -p 9443:9443 -p 8243:8243 wso2/wso2am:4.0.0-rocky
```
...
The above wso2am:4.0.0 image will only be supported for the CPU architecture of your current machine. Docker buildx plugin can be used to build wso2am:4.0.0 image to support any CPU architecture.
...
```
docker buildx build --platform linux/amd64,linux/arm64 -t <DOCKER_USERNAME>/wso2am:4.0.0-rocky --push .
```
...
#### 5. Run
```
docker run -it -p 9443:9443 -p 8243:8243 <DOCKER_USERNAME>/wso2am:4.0.0-rocky
```
