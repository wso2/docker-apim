...
```
docker build -t wso2am:4.6.0-rocky .
```
...
```
docker run -it -p 9443:9443 -p 8243:8243 wso2am:4.6.0-rocky
```
...
The above wso2am:4.6.0-rocky image will only be supported for the CPU architecture of your current machine. Docker buildx plugin can be used to build wso2am:4.6.0-rocky image to support any CPU architecture.
...
```
docker buildx build --platform linux/amd64,linux/arm64 -t <DOCKER_USERNAME>/wso2am:4.6.0-rocky --push .
```
...
#### 5. Run
```
docker run -it -p 9443:9443 -p 8243:8243 <DOCKER_USERNAME>/wso2am:4.6.0-rocky
```
