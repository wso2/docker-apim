...
```
docker build -t wso2am-gw:4.5.0 .
```
...
```
docker run -it -p 8243:8243 wso2am-gw:4.5.0
```
...
The above wso2am-gw:4.5.0 image will only be supported for the CPU architecture of your current machine. Docker buildx plugin can be used to build wso2am-gw:4.5.0 image to support any CPU architecture.
...
```
docker buildx build --platform linux/amd64,linux/arm64 -t <DOCKER_USERNAME>/wso2am-gw:4.5.0 --push .
```
...
#### 5. Run
```
docker run -it -p 9443:9443 -p 8243:8243 <DOCKER_USERNAME>/wso2am-gw:4.5.0
```
