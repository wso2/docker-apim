...
```
docker build -t wso2am-classic-gw:4.7.0-alpine .
```

> By default, the Docker image will prepackage the General Availability (GA) release version of the relevant WSO2 product.

> Note:- wso2am-classic-gw:4.7.0-alpine image can only be built on amd64(x86_64). It is not supported to be built or run natively on Apple Silicon. But it is possible to build an amd64 image using [Docker buildx](https://docs.docker.com/desktop/multi-arch/) and then run via emulation on rosetta. Use following command.

```
docker buildx build --platform linux/amd64 --load  -t wso2am-classic-gw:4.7.0-alpine .
```

#### 3. Running the Docker image.

```
docker run -it -p 8243:8243 wso2am-classic-gw:4.7.0-alpine
```
...
wso2am-classic-gw:4.7.0-alpine
```
